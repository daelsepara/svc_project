...
Only some can copy from ROM (0x08000000-0x0dffffff); none can copy to
8-bit cart RAM (0x0e000000-0x0e00ffff).
...
/* Nametables
   ...
   They may or may not be called "nametables" in official Nintendo
   GBA documentation, but because I come from the NES community,
   and the NES community calls the map data areas "nametables",
...
++++------------- BG X size
*/
#define MOSAIC (*(volatile u16 *)0x04000050)
#define MOSAIC_BG(x,y) ((x) << 12 | (y) << 8)
#define MOSAIC_SPR(x,y) ((x) << 4 | (y))
...
*/
#define BLENDFRAC (*(volatile u16 *)0x04000050)
...
4. Write the stored value to both INTACK and BIOS_INTACK.
